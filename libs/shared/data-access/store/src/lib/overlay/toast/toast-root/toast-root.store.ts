...
import { OverlayToastRootItem, OverlayToastRootState } from './toast-root.state';
...
import { overlayToastRootInitialState } from './toast-root.constants';

@Injectable({
  providedIn: 'root'
})
export class OverlayToastRootComponentStore extends ComponentStore<OverlayToastRootState> {

  constructor() {
    super(overlayToastRootInitialState);
  }

  public readonly push = this.updater<{ position: OverlayCdkPositionModel, item: OverlayToastRootItem }>((state, {
    position,
    item
  }) => {
  ...
  public readonly remove = this.updater<{ position: OverlayCdkPositionModel, close: (componentRef: ComponentRef<any>) => void }>((state
    , { position, close }) => {
